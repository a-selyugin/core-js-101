...
function isTriangle(a, b, c) {
  const arr = [a, b, c].sort((one, two) => one - two);
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  const secNum = a < b ? b : a;
  return `${first}${firstNum}, ${secNum}${last}`;
}
...
function isCreditCardNumber(ccn) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (i % 2 === 1) {
      acc += (str[i] * 2 > 9) ? Number(str[i] * 2 - 9) : Number(str[i] * 2);
    } else acc += Number(str[i]);
  }
  return acc % 10 === 0;
}
...
function getDigitalRoot(num) {
  ...
  while (str.length > 1) {
    str = str.split('').reduce((a, b) => Number(a) + Number(b)).toString();
  }
  return str;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (closeBr.includes(str[i])
        && (compStr[compStr.length - 1] === brackets[str[i]])) {
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < pathes[0].length; i += 1) {
    const compCh = pathes[0][i];
    let common = true;
    for (let j = 0; j < pathes.length; j += 1) {
      if (pathes[j][i] !== compCh) {
        common = false;
      }
    }
    if (common) {
...
function evaluateTicTacToePosition(pos) {
  function Eq(x, y, z) {
    if (x) {
      return (x === 'X' || x === '0') && x === y && x === z;
    }
  }
  for (let i = 0; i < 3; i += 1) {
    if (Eq(pos[0][i], pos[1][i], pos[2][i])) {
      return pos[0][i];
    }

    if (Eq(pos[i][0], pos[i][1], pos[i][2])) {
      return pos[i][0];
    }
  }
  if (Eq(pos[0][0], pos[1][1], pos[2][2])) {
    return pos[0][0];
  }
  if (Eq(pos[0][2], pos[1][1], pos[2][0])) {
    return pos[0][2];
  }
  return undefined;
}
