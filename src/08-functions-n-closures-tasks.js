...
function logger(func, logFunc) {
  return (...args) => {
    logFunc(func.name.concat('(').concat(args.map((i) => JSON.stringify(i))).concat(') starts'));
    logFunc(func.name.concat('(').concat(args.map((i) => JSON.stringify(i))).concat(') ends'));
